...
public class EnemyBehaviour : MonoBehaviour
{
    ...
    void Update()
    {
        if (Vector3.Distance(gameObject.transform.position, preLocation) < 0.07f)
        {
            timer += Time.deltaTime;
        }
        if (!isMoving || timer > 2f)
        {
            timer = 0;
        ...
        preLocation = gameObject.transform.position;
    }
...
//     //     Debug.Log(hit.collider != null && hit.collider.CompareTag("EnemyDamage") && hit.collider.gameObject != this.gameObject);
//     //     return hit.collider != null && hit.collider.CompareTag("EnemyDamage") && hit.collider.gameObject != this.gameObject;
//     // }
//     bool IsBlocked(Vector3 direction)
//     {
//         int layerMask = LayerMask.GetMask("Enemy", "Obstacles"); // Set the layers you want to detect

//         RaycastHit2D hit = Physics2D.Raycast(transform.position, direction, gridSize, layerMask);

//         // Ensure it hit something AND it's NOT this enemy
